		...
		<div class="wrap">
			<div class="con" v-for="(item,index) in recommendList">
				...
				<template v-else>
					...
					<div :style="{background:'url('+recommendList[index+2].tre_img[0].url+')',backgroundSize:'cover'}" class="img" v-if="recommendList[index+2]"></div>
				</template>
			</div>
		</div>
	</div>
...
<script>
	...
	export default{
		...
		mounted:function(){
			axios.post("https://www.jvhv.com/siteindex.php/Trends/Index.html",{
				p:1
			}).then((data)=>{
				console.log(data);
